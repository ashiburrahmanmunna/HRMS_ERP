...
namespace GTERP.Controllers.POS
{
    ...
    public class BOMController : Controller
    {
        ...
        public BOMController(GTRDBContext gtrdb, TransactionLogRepository tran, PermissionLevel pl)
        {
            db = gtrdb;
            tranlog = tran;
            //Repository = rep;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ////Uri url = 
            ///("https://localhost:44336/api/user/GetUsersCompanies"));
            ...
            return View(await db.BOMMain.Include(p => p.Product).Include(u => u.PrdUnit).ToListAsync());
        ...
        public IActionResult Create(BOMMain bOMMain)
        {
            try
            {
            ...
            .Select(x => new { x.Key, x.Value.Errors });
                ...
                {
                    if (bOMMain.BOMMainId > 0)
                    {
                        bOMMain.DateUpdated = DateTime.Now;
                        bOMMain.ComId = (HttpContext.Session.GetString("comid"));

                        if (bOMMain.UserId == null)
                        {
                            bOMMain.UserId = HttpContext.Session.GetString("userid");
                        }
                        bOMMain.UpdatedbyUserId = HttpContext.Session.GetString("userid");

                        foreach (var item in bOMMain.BOMSubs)
                        {
                            item.ComId = (HttpContext.Session.GetString("comid"));
                            if (item.BOMSubId > 0)
                            {
                                item.DateUpdated = DateTime.Now;
                                item.UpdatedbyUserId = HttpContext.Session.GetString("userid");
                                db.Entry(item).State = EntityState.Modified;
                            }
                            else
                            {
                                item.DateAdded = DateTime.Now;
                                item.AddedbyUserId = HttpContext.Session.GetString("userid");
                                db.Entry(item).State = EntityState.Added;
                            }
                        }

                        db.Entry(bOMMain).State = EntityState.Modified;
                        db.SaveChanges();

                        TempData["Status"] = "2";
                        TempData["Message"] = "Data Update Successfully";

                        return Json(new { Success = 2, data = bOMMain.BOMMainId, ex = TempData["Message"].ToString() });
                    }
                    else
                    {
                        bOMMain.UserId = HttpContext.Session.GetString("userid");
                        bOMMain.ComId = (HttpContext.Session.GetString("comid"));
                        bOMMain.DateAdded = DateTime.Now;


                        db.BOMMain.Add(bOMMain);
                        db.SaveChanges();
                        //TempData["Status"] = "1";
                        TempData["Message"] = "Data Save Successfully";

                        return Json(new { Success = 1, data = bOMMain.BOMMainId, ex = TempData["Message"].ToString() }); ;
                    }
                }
                else
                {
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            BOMMain bOMMain = await db.BOMMain.Include(p => p.BOMSubs).
                ThenInclude(p => p.Product)
                .Where(p => p.BOMMainId == id)
                .FirstOrDefaultAsync();
            ...
            InitViewBag("Edit", id, bOMMain);
            //return Json(new {data= purchaseRequisitionMain });
            return View("Create", bOMMain);
        }
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            BOMMain bOMMain = await
                db.BOMMain.Include(p => p.BOMSubs).
                ThenInclude(p => p.Product).Where(p => p.BOMMainId == id).FirstOrDefaultAsync();
            // PurchaseRequisitionMain purchaseRequisitionMain = await db.PurchaseRequisitionMain.FindAsync(id);
            if (bOMMain == null)
            {
                return NotFound();
            }
            InitViewBag("Delete", id, bOMMain);
            //return Json(new {data= purchaseRequisitionMain });
            return View("Create", bOMMain);
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            try
            {
                var bOMMain = await db.BOMMain.FindAsync(id);
                db.BOMMain.Remove(bOMMain);
                await db.SaveChangesAsync();


                TempData["Message"] = "Data Delete Successfully";
                TempData["Status"] = "3";
                tranlog.TransactionLog(RouteData.Values["controller"].ToString(), RouteData.Values["action"].ToString(), TempData["Message"].ToString(), bOMMain.BOMMainId.ToString(), "Delete", bOMMain.BOMMainId.ToString());

                return Json(new { Success = 1, BOMMainId = bOMMain.BOMMainId, ex = "" });
        ...
        private void InitViewBag(string title, int? id = null, BOMMain bomMian = null)
        {
            var comid = HttpContext.Session.GetString("comid");
            ...
            if (title == "Create")
            {
                ...
                ViewData["ProductId"] = new SelectList(db.Products.Include(p => p.vPrimaryCategory)
                        .Where(p => p.vPrimaryCategory.Name.Contains("Chemical Product") || p.vPrimaryCategory.Name.Contains("Packing Materials"))
                        .Select(s => new { Text = s.ProductName + " - [ " + s.ProductCode + " ]", Value = s.ProductId }), "Value", "Text");

                ViewData["PrdUnitId"] = new SelectList(PL.GetPrdUnit(), "PrdUnitId", "PrdUnitName");
                ViewData["WarehouseId"] = new SelectList(PL.GetWarehouse(), "WarehouseId", "WhName");
            }
            else if (title == "Edit" || title == "Delete")
            {
                ...
                ViewData["PrdUnitId"] = new SelectList(PL.GetPrdUnit(), "PrdUnitId", "PrdUnitName", bomMian.PrdUnitId);
                ViewData["WarehouseId"] = new SelectList(PL.GetWarehouse(), "WarehouseId", "WhName");
            }
