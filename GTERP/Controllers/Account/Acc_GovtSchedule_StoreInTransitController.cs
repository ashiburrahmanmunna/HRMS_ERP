...
namespace GTERP.Controllers.Account
{
    ...
    public class Acc_GovtSchedule_StoreInTransitController : Controller
    {
        ...
        public Acc_GovtSchedule_StoreInTransitController(GTRDBContext context, clsProcedure _clsProc, TransactionLogRepository tran)//for report service
        {
        ...
        public ViewResult Index(string FromDate, string ToDate, string criteria)
        {
            ...
            List<Acc_GovtSchedule_StoreInTransit> abcd = new List<Acc_GovtSchedule_StoreInTransit>();

            if (FromDate == null || FromDate == "")
            {

                abcd = db.Acc_GovtSchedule_StoreInTransit.Include(x => x.vAcc_ChartOfAccount).Where(x => x.comid == comid).ToList();
            ...
            //return View(db.Acc_GovtSchedule_StoreInTransit.Where(c => c.GovtScheduleId > 0).ToList());
            return View(abcd);
        ...
        public ActionResult Create()
        {
            ...
            Acc_GovtSchedule_StoreInTransit abc = new Acc_GovtSchedule_StoreInTransit();


            var x = db.Acc_GovtSchedule_StoreInTransit.Where(x => x.comid == comid).OrderByDescending(x => x.GovtScheduleId).FirstOrDefault();
            if (x != null)
            {
        ...
        public ActionResult Create(Acc_GovtSchedule_StoreInTransit Acc_GovtSchedulevar)
        {
            ...
            //if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    db.Acc_GovtSchedule_StoreInTransit.Add(Acc_GovtSchedulevar);
                    db.SaveChanges();


                    //db.Entry(Acc_GovtSchedulevar).GetDatabaseValues();
                    //int id = Acc_GovtSchedule_StoreInTransit.GovtScheduleId; // Yes it's here



                    //db.Categories.Add(Acc_GovtSchedule_StoreInTransit);

                }
            }
            return RedirectToAction("Create");

            //return View(Acc_GovtSchedule_StoreInTransit);
        }
        ...
        public ActionResult Edit(int? id)
        {
            ...
            Acc_GovtSchedule_StoreInTransit Acc_GovtSchedule_StoreInTransit = db.Acc_GovtSchedule_StoreInTransit.Where(c => c.GovtScheduleId == id).FirstOrDefault();


            if (Acc_GovtSchedule_StoreInTransit == null)
            {
                return NotFound();
            }


            ViewData["Criteria"] = new SelectList(CriteriaList, "Value", "Text", Acc_GovtSchedule_StoreInTransit.Criteria);
            var Acc_ChartOfAccount = db.Acc_ChartOfAccounts.Where(c => c.ComId == comid && c.AccId > 0 && c.AccType == "L" && c.AccCode.Contains("2-2-10") || c.AccCode.Contains("2-2-11")); //&& c.ComId == (transactioncomid)
            this.ViewBag.AccId = new SelectList(Acc_ChartOfAccount.Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");

            ViewBag.Title = "Edit";

            return View("Create", Acc_GovtSchedule_StoreInTransit);
        ...
        public JsonResult SetSessionAccountReport(string rptFormat, string FromDate, string ToDate, int? AccId)
        {
            try
            {
                ...
                if (true)
                {
                    ...
                    reportname = "rptGovtStoreInTransitSchedule";
                    filename = "rptGovtStoreInTransitSchedule_" + DateTime.Now.Date;
                    query = "Exec Acc_rptStoreInTransit_Schedule '" + comid + "', '" + FromDate + "' ,'" + ToDate + "' ,'" + AccId + "'  ";
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                db.Acc_GovtSchedule_StoreInTransit.Remove(Acc_GovtSchedule_StoreInTransit);
                db.SaveChanges();

                return Json(new { Success = 1, GovtScheduleId = Acc_GovtSchedule_StoreInTransit.GovtScheduleId, ex = "" });
