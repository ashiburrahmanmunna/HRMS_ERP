...
namespace GTERP.Controllers.Account
{
    ...
    public class AccountsController : Controller
    {
        ...
        public ActionResult FiscalYearList()
        {
            ...
            return View(_fiscalYearRepository.GetFiscalYear());
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult FiscalYearList(Acc_FiscalYear fiscalYear)
        {
        ...
        [HttpPost]
        public IActionResult CreateFiscalYear(Acc_FiscalYear acc)
        {
            Acc_FiscalYear obj = new Acc_FiscalYear();
            var comid = HttpContext.Session.GetString("comid");
            ...
            var prevData = _context.Acc_FiscalYears.OrderByDescending(x => x.FiscalYearId).FirstOrDefault();

            obj.FiscalYearId = prevData.FiscalYearId +1;
            ...
            _context.Acc_FiscalYears.Add(obj);
            _context.SaveChanges();
            return RedirectToAction("FiscalYearList", "Accounts");
        }
        public IActionResult EditFiscalYear(int? id)
        {
            ...
            var data = _context.Acc_FiscalYears.Find(id);
            return View("EditFiscalYear",data);

        }
        [HttpPost]
        public async Task<IActionResult> EditFiscalYear(Acc_FiscalYear acc)
        {
            var data = _context.Acc_FiscalYears.Find(acc.FiscalYearId);
            if (data == null)
            {
                return NotFound();
            }

            //data.OpDate = DateTime.ParseExact(acc.OpDate, "dd-MMM-yyyy", CultureInfo.InvariantCulture).ToString("MMM d yyyy");
            ...
            _context.Acc_FiscalYears.Update(data);
            _context.SaveChanges();
        ...
        public IActionResult DeleteFiscalYear(int? id)
        {
            ...
            var DeleteFiscalYear = _context.Acc_FiscalYears.Find(id);
        ...
        public IActionResult DeleteFiscalYear(int id)
        {
            try
            {
                var fiscalyear = _context.Acc_FiscalYears.Find(id);
                _fiscalYearRepository.Delete(fiscalyear);
                _context.SaveChanges();
        ...
        public JsonResult GetMonthList(int? id)
        {
            ...
            if (id == null)
            {
                List<Acc_FiscalYear> fiscalyear = _accountProcessRepository.FiscalYear();
                id = fiscalyear.Max(p => p.FYId);
            }

            List<Acc_FiscalMonth> Acc_FiscalMonth = _accountProcessRepository.Acc_FiscalMonth(id);
        ...
        public JsonResult SetProcess(string[] monthid, string criteria, int? Currency, string FYId, string MinAccCode, string MaxAccCode)
        {
            ...
            _accountProcessRepository.SetProcess(monthid, criteria, Currency, FYId, MinAccCode, MaxAccCode);
        ...
        public IActionResult AccountProcessList()
        {
            ...
            string comid = HttpContext.Session.GetString("comid");
            int defaultcountry = _accountProcessRepository.DefaultCountry();

            ViewBag.CountryId = _accountProcessRepository.CountryId();

            Acc_AccProcessViewModel model = new Acc_AccProcessViewModel();
            List<Acc_FiscalYear> fiscalyear = _accountProcessRepository.FiscalYear();
            int fiscalyid = fiscalyear.Max(p => p.FYId);

            List<Acc_FiscalMonth> fiscalmonth = _accountProcessRepository.FiscalMonth();
            ...
            TempData["Status"] = "2";
            TempData["Message"] = "Account Processing Successfully";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AccountProcessList(Acc_AccProcessViewModel model, string criteria, string command)
        {
            try
            {
                if (command != "Save")
        ...
        public string prcDataSave(Acc_AccProcessViewModel model, string criteria)
        {
            ...
            try
            {
                _accountProcessRepository.prcDataSave(model, criteria);
