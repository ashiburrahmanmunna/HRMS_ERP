...
namespace GTERP.Controllers.Account
{
    ...
    public class Acc_ChartOfAccountPFsController : Controller
    {
        ...
        public Acc_ChartOfAccountPFsController(GTRDBContext _db, CommercialRepository rep, TransactionLogRepository tran)
        {
            tranlog = tran;
            db = _db;
            Repository = rep;
       ...
       // [OverridableAuthorize]
        public ActionResult Index()
        {
            ...
            List<Acc_ChartOfAccount_PF> chartofaccountlist = db.Acc_ChartOfAccounts_PF.Take(1).Include(x => x.ParentChartOfAccount).Where(c => c.AccId > 0 && c.comid == comid && c.IsSysDefined == 0).ToList();

            return View(chartofaccountlist);
            //modelList = Acc_ChartOfAccount_PF.prcGetData();
            //return View(modelList);
        }

        public ActionResult COATreePF()
        {
            var comid = HttpContext.Session.GetString("comid");
            //List<Acc_ChartOfAccount_PF> all = new List<Acc_ChartOfAccount_PF>();
            //using (GTRDBContext db = new GTRDBContext())
            //{
               var all = db.Acc_ChartOfAccounts_PF.Where(a => a.comid == comid).OrderBy(a => a.ParentID).ToList();
            //}
            return View(all);
        }
       ...
       // [OverridableAuthorize]
        public ActionResult Details(int Id)
        {
            ...
            //}
            //prcLoadCombo();
            //return View(Acc_ChartOfAccount_PF.prcGetData(Id));
            return View();
       ...
       // [OverridableAuthorize]
        public ActionResult Create()
        {
            try
            {
                ...
                string comid = HttpContext.Session.GetString("comid");
                var model = new Acc_ChartOfAccount_PF();


                var lastacccoa = db.Acc_ChartOfAccounts_PF.Where(x => x.comid == comid).OrderByDescending(x => x.AccId).FirstOrDefault();
                ...
                //ViewBag.ParentIdId = new SelectList(db.Acc_ChartOfAccount_PF, "AccId", "AccName").ToList();

                //Acc_ChartOfAccount_PF model = new Acc_ChartOfAccount_PF();
                //ViewBag.ParentId = new SelectList(db.Acc_ChartOfAccount_PF, "AccId", "AccName").ToList();

                
                SqlParameter[] sqlParameter = new SqlParameter[1];
                sqlParameter[0] = new SqlParameter("@comid", comid);

                List<COAtemp> COAParent = Helper.ExecProcMapTList<COAtemp>("Acc_prcgetCOAParent_PF", sqlParameter);

                //List<COAtemp> COAParent = (db.Database.SqlQuery<COAtemp>("[prcgetCOAParent_PF]  @comid", new SqlParameter("comid", HttpContext.Session.GetString("comid")))).ToList();
                ...
                ViewBag.CountryId = new SelectList(db.Countries, "CountryId", "CurrencyShortName", defaultcurrency).ToList();
                //ViewBag.ParentId = db.Acc_ChartOfAccounts_PF.Where(m => m.AccType == "G").ToList();
       ...
       // [OverridableAuthorize]
        ...
        public ActionResult Create(Acc_ChartOfAccount_PF model, string title)
        {
                ...
                //if (ModelState.IsValid)
                {
                    if (model.AccId > 0)
                    {
                        ...
                        if (isModelBase == 1)
                        {
                            ...
                            {
                                var parentcodeinfo = db.Acc_ChartOfAccounts_PF.Where(x => x.AccId == model.ParentID && x.comid == comid).FirstOrDefault();
                                string maxacccsubid = db.Acc_ChartOfAccounts_PF.Where(x => x.ParentID == model.ParentID && x.comid == comid).Max(x => x.AccSubId).ToString();

                                if (model.AccType != model.PrevAccType)
                                {


                                    int ExistAccIdVoucher = db.Acc_VoucherSub_PF.Where(x => x.AccId == model.AccId).Count();
                                    int ExistAccIdSubAccountsHead = db.Acc_ChartOfAccounts_PF.Where(x => x.ParentID == model.AccId && x.comid == comid).Count();
                                    ...
                                    if (ExistAccIdVoucher > 0)
                                    {
                                        ...
                                        ViewBag.CountryId = new SelectList(db.Countries, "CountryId", "CurrencyShortName", model.comid);
                                ...
                                if (maxacccsubid == null || maxacccsubid == "")
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        List<COAtemp> COAParent = Helper.ExecProcMapTList<COAtemp>("Acc_prcgetCOAParent_PF", sqlParameter).ToList();
                            ...
                            if (title == "Delete")
                            {
                                ...
                                var count = db.Acc_ChartOfAccounts_PF.Where(m => m.ParentID == model.AccId && m.comid == comid).ToList();
                    ...
                    else
                    {
                        ...
                        if (isModelBase == 1)
                        {
                            ...
                            db.Acc_ChartOfAccounts_PF.Add(model);
                            db.SaveChanges();
       ...
       // [OverridableAuthorize]
        public ActionResult Edit(int ?Id)
        {
            ...
            Acc_ChartOfAccount_PF chartofaccount = db.Acc_ChartOfAccounts_PF.Find(Id);
            ...
            //ViewBag.ParentIdId = new SelectList(db.Acc_ChartOfAccount_PF, "AccId", "AccName").ToList();
            //List<COAtemp> COAParent = (db.Database.SqlQuery<COAtemp>("[prcgetCOAParent_PF]  @comid", new SqlParameter("comid", Session["comid"]))).ToList();
            ...
            ViewBag.CountryId = new SelectList(db.Countries, "CountryId", "CurrencyShortName" , chartofaccount.CountryID).ToList();
       ...
       // [OverridableAuthorize]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(Acc_ChartOfAccount_PF model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                if (isModelBase == 1)
                {
                    ...
                    Acc_ChartOfAccount_PF coa = db.Acc_ChartOfAccounts_PF.Where(c => c.comid == (HttpContext.Session.GetString("comid")) && c.AccId == id).FirstOrDefault();

                    int ExistAccIdVoucher = db.Acc_VoucherSub_PF.Where(x => x.AccId == coa.AccId).Count();
                    int ExistAccIdSubAccountsHead = db.Acc_ChartOfAccounts_PF.Where(x => x.ParentID == coa.AccId && x.comid == comid).Count();
                    ...
                    {
                        db.Acc_ChartOfAccounts_PF.Remove(coa);
                        db.SaveChanges();
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                var query = from e in db.Acc_ChartOfAccounts_PF.Where(x => x.AccId > 0 && x.IsSysDefined == 0 && x.comid == comid).OrderByDescending(x => x.AccId)
        ...
        public string prcDataSave(Acc_ChartOfAccount_PF model)
        {
        ...
        public string prcUpdateData(Acc_ChartOfAccount_PF model)
        {
            //ArrayList arQuery = new ArrayList();
            //clsProcedure clsProc = new clsProcedure();
            //clsConnectionNew clsCon = new clsConnectionNew(Session["dbACC"].ToString(), true);
        ...
        public string prcDeleteData(Acc_ChartOfAccount_PF model)
        {
            //ArrayList arQuery = new ArrayList();
            //clsProcedure clsProc = new clsProcedure();
            //clsConnectionNew clsCon = new clsConnectionNew(Session["dbACC"].ToString(), true);
        ...
        [HttpPost, ActionName("AccountsReport_PF")]
        public JsonResult AccountsReport_PF(string rptFormat, string action)
        {
            try
            {
                string comid = HttpContext.Session.GetString("comid");
                ...
                if (action == "PrintAccCOA")
                {
                    ...
                    reportname = "rptCOA_PF";
                    filename = "rptCOA_PF_List" + DateTime.Now.Date.ToString();
                    var query = "Exec [Acc_rptCOA_PF] '" + comid + "'";
                    HttpContext.Session.SetString("reportquery", "Exec [Acc_rptCOA_PF] '" + comid + "'");
