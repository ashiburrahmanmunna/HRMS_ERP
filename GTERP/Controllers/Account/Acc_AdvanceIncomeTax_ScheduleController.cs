...
namespace GTERP.Controllers.Account
{
    ...
    public class Acc_AdvanceIncomeTax_ScheduleController : Controller
    {
        ...
        public Acc_AdvanceIncomeTax_ScheduleController(GTRDBContext context, clsProcedure _clsProc, TransactionLogRepository tran)//for report service
        {
        ...
        public ViewResult Index(string FromDate, string ToDate, string criteria)
        {
            ...
            List<Acc_AdvanceIncomeTax_Schedule> abcd = new List<Acc_AdvanceIncomeTax_Schedule>();

            if (FromDate == null || FromDate == "")
            {

                abcd = db.Acc_AdvanceIncomeTax_Schedule.Include(x => x.vAcc_ChartOfAccount).Where(x => x.comid == comid).ToList();
            ...
            //return View(db.Acc_AdvanceIncomeTax_Schedule.Where(c => c.GovtScheduleId > 0).ToList());
            return View(abcd);
        ...
        public ActionResult Create()
        {
            ...
            Acc_AdvanceIncomeTax_Schedule abc = new Acc_AdvanceIncomeTax_Schedule();


            var x = db.Acc_AdvanceIncomeTax_Schedule.Where(x => x.comid == comid).OrderByDescending(x => x.GovtScheduleId).FirstOrDefault();
            if (x != null)
            {
        ...
        public ActionResult Create(Acc_AdvanceIncomeTax_Schedule Acc_GovtSchedulevar)
        {
            ...
            //if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    db.Acc_AdvanceIncomeTax_Schedule.Add(Acc_GovtSchedulevar);
                    db.SaveChanges();


                    //db.Entry(Acc_GovtSchedulevar).GetDatabaseValues();
                    //int id = Acc_AdvanceIncomeTax_Schedule.GovtScheduleId; // Yes it's here



                    //db.Categories.Add(Acc_AdvanceIncomeTax_Schedule);

                }
            }
            return RedirectToAction("Create");

            //return View(Acc_AdvanceIncomeTax_Schedule);
        }
        ...
        public ActionResult Edit(int? id)
        {
            ...
            Acc_AdvanceIncomeTax_Schedule Acc_AdvanceIncomeTax_Schedule = db.Acc_AdvanceIncomeTax_Schedule.Where(c => c.GovtScheduleId == id).FirstOrDefault();


            if (Acc_AdvanceIncomeTax_Schedule == null)
            {
                return NotFound();
            }


            ViewData["Criteria"] = new SelectList(CriteriaList, "Value", "Text", Acc_AdvanceIncomeTax_Schedule.Criteria);
            var Acc_ChartOfAccount = db.Acc_ChartOfAccounts.Where(c => c.ComId == comid && c.AccId > 0 && c.AccType == "L" && c.AccCode.Contains("2-2-10") || c.AccCode.Contains("2-2-11")); //&& c.ComId == (transactioncomid)
            this.ViewBag.AccId = new SelectList(Acc_ChartOfAccount.Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");

            ViewBag.Title = "Edit";

            return View("Create", Acc_AdvanceIncomeTax_Schedule);
        ...
        public JsonResult SetSessionAccountReport(string rptFormat, string FromDate, string ToDate, int? AccId)
        {
            try
            {
                ...
                if (true)
                {
                    ...
                    reportname = "rptAdvanceIncomeTaxSchedule";
                    filename = "rptAdvanceIncomeTaxSchedule_" + DateTime.Now.Date;
                    query = "Exec Acc_rptAdvanceIncomeTax_Schedule '" + comid + "', '" + FromDate + "' ,'" + ToDate + "' ,'" + AccId + "'  ";
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                db.Acc_AdvanceIncomeTax_Schedule.Remove(Acc_AdvanceIncomeTax_Schedule);
                db.SaveChanges();

                return Json(new { Success = 1, GovtScheduleId = Acc_AdvanceIncomeTax_Schedule.GovtScheduleId, ex = "" });
