...
namespace GTERP.Controllers.Account
{
    ...
    public class Acc_GovtSchedule_LoanController : Controller
    {
        ...
        public Acc_GovtSchedule_LoanController(GTRDBContext context, clsProcedure _clsProc, TransactionLogRepository tran)//for report service
        {
        ...
        public ViewResult Index(string FromDate, string ToDate, string criteria)
        {
            ...
            var Acc_ChartOfAccount = db.Acc_ChartOfAccounts.Where(c => c.ComId == comid && c.AccId > 0 && c.AccType == "L" && c.AccCode.Contains("2-2-15")); //&& c.ComId == (transactioncomid)
            ...
            List<Acc_GovtSchedule_Loan> abcd = new List<Acc_GovtSchedule_Loan>();

            if (FromDate == null || FromDate == "")
            {

                abcd = db.Acc_GovtSchedule_Loans.Include(x => x.vAcc_ChartOfAccount).Where(x => x.comid == comid).ToList();
            ...
            //return View(db.Acc_GovtSchedule_Loans.Where(c => c.GovtScheduleId > 0).ToList());
            return View(abcd);
        ...
        public ActionResult Create()
        {
            ...
            Acc_GovtSchedule_Loan abc = new Acc_GovtSchedule_Loan();


            var x = db.Acc_GovtSchedule_Loans.Where(x => x.comid == comid).OrderByDescending(x => x.GovtScheduleId).FirstOrDefault();
            if (x != null)
            {
                ...
                abc.Principle = x.Principle;
                abc.TotalLoan = x.TotalLoan;
        ...
        public ActionResult Create(Acc_GovtSchedule_Loan Acc_GovtSchedulevar)
        {
            ...
            //if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    db.Acc_GovtSchedule_Loans.Add(Acc_GovtSchedulevar);
                    db.SaveChanges();


                    //db.Entry(Acc_GovtSchedulevar).GetDatabaseValues();
                    //int id = Acc_GovtSchedule_Loans.GovtScheduleId; // Yes it's here



                    //db.Categories.Add(Acc_GovtSchedule_Loan);

                }
            }
            return RedirectToAction("Create");

            //return View(Acc_GovtSchedule_Loan);
        }
        ...
        public ActionResult Edit(int? id)
        {
            ...
            Acc_GovtSchedule_Loan Acc_GovtSchedule_Loan = db.Acc_GovtSchedule_Loans.Where(c => c.GovtScheduleId == id).FirstOrDefault();


            if (Acc_GovtSchedule_Loan == null)
            {
                return NotFound();
            }


            ViewData["Criteria"] = new SelectList(CriteriaList, "Value", "Text", Acc_GovtSchedule_Loan.Criteria);
            var Acc_ChartOfAccount = db.Acc_ChartOfAccounts.Where(c => c.ComId == comid && c.AccId > 0 && c.AccType == "L" && c.AccCode.Contains("2-2-15")); //&& c.ComId == (transactioncomid)
            this.ViewBag.AccId = new SelectList(Acc_ChartOfAccount.Select(s => new { Text = s.AccName + " - [ " + s.AccCode + " ]", Value = s.AccId }).ToList(), "Value", "Text");

            ViewBag.Title = "Edit";

            return View("Create", Acc_GovtSchedule_Loan);
        ...
        public JsonResult SetSessionAccountReport(string rptFormat, string FromDate, string ToDate, int? AccId)
        {
            try
            {
                ...
                if (true)
                {
                    ...
                    reportname = "rptGovtLoanSchedule";
                    filename = "rptGovtLoanSchedule_" + DateTime.Now.Date;
                    query = "Exec Acc_rptGovtLoan_Schedule '" + comid + "', '" + FromDate + "' ,'" + ToDate + "' ,'" + AccId + "'  ";
        ...
        public JsonResult DeleteConfirmed(int? id)
        {
            try
            {
                ...
                db.Acc_GovtSchedule_Loans.Remove(Acc_GovtSchedule_Loan);
                db.SaveChanges();

                return Json(new { Success = 1, GovtScheduleId = Acc_GovtSchedule_Loan.GovtScheduleId, ex = "" });
